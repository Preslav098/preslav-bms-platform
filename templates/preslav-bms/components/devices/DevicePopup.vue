...
const livePower = computed<number | null>(() => {
    return firstFiniteNumber(
        ...
        readNestedMetric(liveStatus.value, ['apower', 'power', 'act_power']),
...
const liveVoltage = computed<number | null>(() => {
    return firstFiniteNumber(
        readNestedMetric(liveStatus.value, ['voltage']),
...
const liveCurrent = computed<number | null>(() => {
    return firstFiniteNumber(
        readNestedMetric(liveStatus.value, ['current']),
...
const totalEnergyWh = computed<number | null>(() => {
    return firstFiniteNumber(
        readNestedMetric(liveStatus.value, ['total_act_energy', 'total_energy', 'total']),
...
const liveTemperature = computed<number | null>(() => {
    return firstFiniteNumber(
        readNestedMetric(liveStatus.value, ['tC', 'temperature', 'temp']),
...
const liveHumidity = computed<number | null>(() => {
    return firstFiniteNumber(
        readNestedMetric(liveStatus.value, ['rh', 'humidity']),
...
const liveBattery = computed<number | null>(() => {
    return firstFiniteNumber(
        readNestedMetric(liveStatus.value, ['battery', 'battery_pct', 'percent']),
...
const liveRssi = computed<number | null>(() => {
    return firstFiniteNumber(
        readNestedMetric(liveStatus.value, ['rssi']),
...
async function refreshLive(): Promise<void> {
    ...
    // Read the current device state directly through the Host SDK. This keeps
    // relay, voltage/current, BLU battery/RSSI and sensor values live even when
    // the historical metrics pipeline has not produced a fresh bucket yet.
    try {
        const status = await hostDevicesApi.call(
            shellyId.value,
            'Shelly.GetStatus',
            {}
        );
        if (status && typeof status === 'object') {
            liveStatus.value = status as Record<string, unknown>;
        }
    } catch (error) {
        console.debug('Direct live status is unavailable; using host data.', error);
    }
    ...
    try {
        ...
        liveEnergyWatts.value = firstFiniteNumber(
            readNestedMetric(liveStatus.value, ['apower', 'power', 'act_power']),
            row?.watts,
            response.watts
        );
    } catch (error) {
        liveEnergyWatts.value = readNestedMetric(liveStatus.value, ['apower', 'power', 'act_power']);
        if (liveEnergyWatts.value === null) throw error;
    }
}
